    ...
    def interpret(self, x):
        ...
        for l in range(self.visualize_layer, L)[::-1]:
            ...
            if l > round(0.6 * L):      #LRP-0: upper layers
                rho_p = lambda p: p;         incr_p = lambda z: z+1e-9
                rho_n = lambda p: p;         incr_n = lambda z: z+1e-9
            if round(0.6 * L) >= l > round(0.3 * L): #LRP-epsilon: middle layers
                rho_p = lambda p: p;         incr_p = lambda z: z+1e-9+0.1*((z**2).mean()**.5).detach()
                rho_n = lambda p: p;         incr_n = lambda z: z+1e-9+0.1*((z**2).mean()**.5).detach()
